...
@HiltViewModel
class SettingsAdOrderEditViewModel @Inject constructor(private val repository: Repository, private val adRepository: AdRepository) : BaseViewModel() {
    ...
    suspend fun updateData(id: Int, order: Int) {
        var result = viewModelScope.async {
            adRepository.updateData(id, order)
            adList.setValue(adRepository.getOrderAscList())
        }
        return result.await()
    }
